...
def aws_credentials():
    ...
    os.environ['JWT_SECRET_NAME'] = "jwtSecret"
...
@pytest.fixture(scope="function")
def secrets_manager_resource(aws_credentials):
    with mock_secretsmanager():
        client = boto3.client('secretsmanager', region_name='us-west-2')
        client.create_secret(Name="jwtSecret", SecretString='{"JWT_SECRET":"test123"}')
        yield client

@pytest.fixture(scope="function")
def jwt_manager(secrets_manager_resource):
    return JWTManager(secrets_client=secrets_manager_resource,secret_name = os.environ['JWT_SECRET_NAME'])
...
def api_event():
    ...
    return {
        ...
        "GET": {
            "httpMethod": "GET",
            "resource": "/api/{app_name}/version",
            "pathParameters": {"app_name": "TestApp"}
        },
        "POST_BUMP": {
            "httpMethod": "POST",
            "resource": "/api/{app_name}/bump",
            "pathParameters": {"app_name": "TestApp"},
            "queryStringParameters": {"type": "minor"}
        },
        "POST_SET": {
            "httpMethod": "POST",
            "resource": "/api/{app_name}/set",
            "pathParameters": {"app_name": "TestApp"},
...
def test_lambda_handler_create_app(db_operations, api_event):
    ...
    response = lambda_handler(event=api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_lambda_handler_get_version(db_operations, api_event):
    ...
    lambda_handler(event=api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    # Now get the version
    response = lambda_handler(event=api_event["GET"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_lambda_handler_bump_version(db_operations, api_event):
    ...
    lambda_handler(event=api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)  # Create the app first
    response = lambda_handler(event=api_event["POST_BUMP"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_lambda_handler_set_version(db_operations, api_event):
    ...
    lambda_handler(event=api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)  # Create the app first
    response = lambda_handler(event=api_event["POST_SET"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_lambda_handler_secure_create_app(db_operations, api_event, jwt_manager):
    """Test creating a new app entry with secure 'true'. This should create a JWT as well."""
    response = lambda_handler(event=api_event["POST_SECURE"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    assert response['statusCode'] == 201, f"Status code should be 201 for successful creation. Response: {response}"
    assert 'token' in response['body'], f"Response body should contain a JWT token. Response body: {response['body']}"

def test_lambda_handler_secure_get_version(db_operations, api_event, jwt_manager):
    ...
    # Now get the version
    response = lambda_handler(event=api_event["GET_SECURE"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    assert response['statusCode'] == 200, f"Status code should be 200 for successful fetch. Response: {response}"
    assert json.loads(response['body'])['version'] == '0.1.0', f"The version should be 0.1.0 in the response body. Response body: {response['body']}"

def test_lambda_handler_secure_bump_version(db_operations, api_event, jwt_manager):
    ...
    response = lambda_handler(event=api_event["POST_BUMP_SECURE"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    assert response['statusCode'] == 200, f"Status code should be 200 for successful version bump. Response: {response}"
    assert json.loads(response['body'])['new_version'] == '0.2.0', f"Version should be bumped to 0.2.0. Response body: {response['body']}"

def test_invalid_jwt(db_operations, api_event, jwt_manager):
    """Test accessing secure endpoints with an invalid JWT."""
    # First create a secure app
    response_create = lambda_handler(api_event["POST_SECURE"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    token = json.loads(response_create['body'])['token'] + "tampered"

    # Use the tampered token to fetch version
    api_event["GET_SECURE"]["headers"]["Authorization"] = token
    response = lambda_handler(api_event["GET_SECURE"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_create_app_with_invalid_app_name(db_operations, api_event, invalid_name):
    ...
    response = lambda_handler(api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_set_invalid_version_format(db_operations, api_event):
    ...
    # First create the app
    lambda_handler(api_event["POST"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
    api_event["POST_SET"]["queryStringParameters"]["new_version"] = "1..0"
    response = lambda_handler(api_event["POST_SET"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_unsupported_http_method(db_operations, api_event):
    ...
    response = lambda_handler(api_event["GET"], context=None, db_operations=db_operations, jwt_manager=jwt_manager)
...
def test_create_app_with_custom_jwt_expiry(db_operations, api_event, jwt_manager):
